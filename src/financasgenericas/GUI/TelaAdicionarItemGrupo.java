    ...
    private void contruirTela() {
        ...
        txtValorItem.addFocusListener(new FocusAdapter() {
            ...
            public double formataSemArendodamento(double valor) {
                ...
                char[] cNovoValor = new char[cValor.length];
                    ...
                    for (int i = 0; i < ckParticipantes.length; i++) {
                        if (ckParticipantes[i].isSelected()) {
                            if (diff.compareTo(BigDecimal.ZERO) > 0) {
                                txtValorPorParticipante[i].setText(Double.toString(valorPorPessoa+ 0.01) );
                            }else{
                                txtValorPorParticipante[i].setText(Double.toString(valorPorPessoa));
                            }
    ...
    private boolean verificaSomaDosValoresPagos() {
        ...
        for (int i = 0; i < ckParticipantes.length; i++) {
            if (!txtValorPorParticipante[i].getText().isEmpty()) {
                soma += Double.parseDouble(txtValorPorParticipante[i].getText());
            }
        }
        return soma == Double.parseDouble(txtValorItem.getText());
    }
