 ...
 * @author Otavio
 */
public class TelaContasPendentes extends TelaLogado {
    ...
    public TelaContasPendentes() {
        super("Contas pendentes");
        construirTela();
        pack();
        ...
        if (despesas != null) {
            for (Map.Entry<Long, Despesa> entry : despesas.entrySet()) {
                ...
                if (value instanceof DespesaGrupo) {
                    if (((DespesaGrupo) value).valorAReceberPeloIntegrante(ControlerUsuario.getUsuario(UsuarioLogado.getInstance().getUsuario())) != 0.0 || ((DespesaGrupo) value).valorAPagarPeloIntegrante(ControlerUsuario.getUsuario(UsuarioLogado.getInstance().getUsuario())) != 0.0) {
                        obj[0] = value.getNome();
                ...
                if (value instanceof DespesaGrupo) {
                    System.out.println(((DespesaGrupo) value).valorAReceberPeloIntegrante(ControlerUsuario.getUsuario(UsuarioLogado.getInstance().getUsuario())));
                    System.out.println(((DespesaGrupo) value).valorAPagarPeloIntegrante(ControlerUsuario.getUsuario(UsuarioLogado.getInstance().getUsuario())));
